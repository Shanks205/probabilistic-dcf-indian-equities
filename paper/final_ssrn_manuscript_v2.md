...
This working paper develops a probabilistic discounted cash flow framework for a 12-company Indian listed-equity sample. The project applies free cash flow to firm valuation, scenario-based operating assumptions, WACC normalization, terminal value sensitivity, enterprise-value bridge reconciliation and peer multiple triangulation. The objective is to separate business quality from valuation-expectation risk. Rather than presenting a single target price, the study compares market price with a distribution of intrinsic value outcomes.
...
This project therefore applies a probabilistic FCFF framework to 12 selected Indian listed companies:
...
The paper deliberately avoids presenting its outputs as buy/sell recommendations. Instead, it treats valuation as a probability-weighted research process.
...
## 4. Annual-Report Evidence Summary
...
The remaining automated gaps were filled manually for Siemens India, Bharat Dynamics, Data Patterns, Voltamp Transformers and Bharat Bijlee, and those manual patches are documented in:
...
### 5.2 Peer Median Summary

The company-level peer medians are summarized below:
...
## 6. Draft Exhibits and Interpretation
...
These charts compare market prices with median fair-value outputs and implied margins of safety. They should be rerun after final EV and WACC assumptions are locked.
...
1. Some balance-sheet note pages still require final audit-grade locking for cash, investments, debt and share capital.
...
The relative valuation layer supports the framework by providing peer-context triangulation, while the annual-report mapping and EV bridge notes improve evidence quality. The remaining work before final SSRN publication is to lock final balance-sheet note references, rerun final charts after final assumptions and prepare the final SSRN submission packet.

## Appendix A: Key Repository Files
